...
class GripperASynch {
 public:
   ...
   * @throw IncompatibleVersionException if this version of `libfranka` is not supported.
   */
  explicit GripperASynch(const std::string& franka_address);
   ...
   * @param[in] gripper Other Gripper instance.
   */
  GripperASynch(GripperASynch&& GripperASynch) noexcept;
   ...
   * @return Model instance.
   */
  GripperASynch& operator=(GripperASynch&& gripper) noexcept;

  /**
   * Closes the connection.
   */
  ~GripperASynch() noexcept;
  ...
  /// @cond DO_NOT_DOCUMENT
  GripperASynch(const GripperASynch&) = delete;
  GripperASynch& operator=(const GripperASynch&) = delete;
  /// @endcond
